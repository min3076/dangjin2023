...
$(document).ready(function() {
	var bannerNum = $("#open_popup_wrap1 ul li").size();//입학홈페이지 팝업존 수
...
function popup_slide (bannerNum){
	...
	$(document).ready(function() {
		if($("#wrap").attr("class") == "sub"){
			...
			$("#open_popup_wrap1").stop().animate({ height: 0}, 300, function(){
				$("#open_popup_wrap1").css("display", "none");
		...
		function pzOpen(){
				...
				$("#open_popup_wrap1").css("display", "block");
				$("#open_popup_wrap1").stop().animate({height: 390}, 300);
				$("#wrap .pop_btn").hide();
				$("#wrap .pop_btn_x").show();
		...
		$("#open_popup_wrap1 .close_btn").click(function(event){
			(event.preventDefault) ? event.preventDefault() : event.returnValue = false;
			$("#open_popup_wrap1").stop().animate({ height: 0}, 300, function(){
				$("#open_popup_wrap1").css("display", "none");
			});	
