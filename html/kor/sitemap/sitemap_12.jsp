...
<%
	String id2 = "DNJ003";
	String reqNum2 = "";
	String retUrl2 = "23https://www.dangjin.go.kr/siren24/ipin_result_seed.jsp";
...
<script type="text/javascript" language="javascript">
	...
	function openPCCWindow() {
		...
		if($("input[name=child_nm10]").length > 0){
	  		if($("input[name=child_nm10]").val() == ""){
	  			alert("14세 미만 아동 이름을 입력하세요.");
	  			$("input[name=child_nm10]").focus();
	  			return false;
	  		}
	  	}

		$("input[name=child_nm1]").val($("input[name=child_nm10]").val());
...
<script>
	...
	function openCBAWindow() {
		...
		if($("input[name=child_nm20]").length > 0){
	  		if($("input[name=child_nm20]").val() == ""){
	  			alert("14세 미만 아동 이름을 입력하세요.");
	  			$("input[name=child_nm20]").focus();
	  			return false;
	  		}
	  	}

		$("input[name=child_nm2]").val($("input[name=child_nm20]").val());
	...
	$(function(){
		$("#certifiedChild").click(function(){
			if($(this).is(":checked") == true){
				...
				$(".childNm1").append("<p class='btn_l' style='display: block;'><label for='childNm10' class='flow' style='position: absolute; visibility: visible; font-size: 0;'>14세 미만 아동 이름 입력</label><input name='child_nm10' type='text' id='childNm10' class='input_blue' value='' placeholder='14세 미만 아동 이름'></p>");
				
				$(".childNm2").append("<p class='color-orange mt_10'><b>14세 미만 본인 인증 시 법정대리인의 휴대폰으로 인증 바랍니다.</b></p>");
				$(".childNm2").append("<p class='btn_l' style='display: block;'><label for='childNm20' class='flow' style='position: absolute; visibility: visible; font-size: 0;'>14세 미만 아동 이름 입력</label><input name='child_nm20' type='text' id='childNm20' class='input_blue' value='' placeholder='14세 미만 아동 이름'></p>");
				
				$(".childNm3").append("<p class='color-orange mt_10'><b>14세 미만 본인 인증 시 법정대리인의 I-PIN으로 인증 바랍니다.</b></p>");
				$(".childNm3").append("<p class='btn_l' style='display: block;'><label for='childNm3' class='flow' style='position: absolute; visibility: visible; font-size: 0;'>14세 미만 아동 이름 입력</label><input name='child_nm3' type='text' id='childNm3' class='input_blue' value='' placeholder='14세 미만 아동 이름'></p>");
...
<form name="reqPCCForm" method="post" onsubmit="return openPCCWindow()">
	...
	<input type="hidden" name="child_nm1" value="">
</form>

<form name="reqCBAForm" method="post" onsubmit="return openCBAWindow()">
	<input type="hidden" name="reqInfo" value="<%=reqInfo2%>"> 
	<input type="hidden" name="retUrl" value="<%=retUrl2%>"> 
	<input type="hidden" name="returnUrl" value="<%=returnUrl%>"> 
	<input type="hidden" name="verSion" value="2"> 
	<input type="hidden" name="child_nm2" value="">
</form>
...
<div class="uit3">
	<div class="inner_normal icowrap">
		...
		<p class="mt_20">
			...
			<label for="certifiedChild">만 14세 미만 시민 인증하기</label>
		</p>
	</div>
</div>
	...
	<div class="terms-content" tabindex="0">
		...
		<table class="tbl_basic center mt_10">
			<caption>
				...
				<details>
					<p>제공 받는 자, 제공받는 자의 개인정보 이용 목적, 제공하는 개인정보의 항목, 제공받는
			...
			<tbody>
				<tr>
					<th scope="row">서울 신용 평가정보(주)</th>
					<td>본인확인 인증</td>
					<td>
						<h4 class="color-blue mt_20">이름, 성별, 생년월일, 휴대폰번호, 14세 미만 아동의 법정대리인의 정보</h4>
					</td>
					<td>이미 보유하고 있는 개인정보이므로 별도 저장하지 않음</td>
				</tr>
				<tr>
					<th scope="row">SCI평가정보</th>
					<td>I-PIN 인증</td>
					<td>
						<h4 class="color-blue mt_20">I-PIN(개인식별번호), 14세 미만 아동의 법정대리인 정보(I-PIN(개인식별번호))</h4>
					</td>
					<td>이미 보유하고 있는 개인정보이므로 별도 저장하지 않음</td>
				</tr>
			...
			<div class="text_box">
				...
				<div class="tipbox">
					...
					<ul class="list_1st">
						...
						<li>이용문의 1533-3713 (운영시간 : 평일, 9:00~18:00)</li>
